...
class MainApp(QMainWindow):
    ...
    def create_menu(self):
        ...
        self.switch_to_project_action.setShortcut("Ctrl+Shift+P")
        ...
        self.switch_to_retouch_action.setShortcut("Ctrl+Shift+R")
...
''')
    ...
    if filename:
        ...
        if len(filenames) == 1 and extension == 'fsp':
            if args.start:
                main_app.project_window.open_project_and_run(first_filename)
            else:
        ...
        if args.start:
            QTimer.singleShot(
                100, lambda: main_app.project_window.new_project_silent_and_run(args.path))
        else:
            QTimer.singleShot(
                100, lambda: main_app.project_window.new_project(args.path))
