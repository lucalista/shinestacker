...
* ```i_max``` (optional): if specifies, only pixels with content less pr equal tham ```i_max``` are used. It may be useful to remove white areas. Not used if ```MATCH_HIST``` is specified as value for ```corr_map```. Note that for 8-bit images ```i_max``` should be less or equal to 255, while for 16-bit images ```i_max``` should be less or equal to 65535.
...
* Even if ```exif_path``` is explicitly specified, for 16-bit TIFF exif data are not be saved.
